...
int main(int argc, const char *argv[])
{
    ...
    Timer t("FULL TIMER");
    int dof = 6;
    std::string line = "../../DATASET/SCENE_Fabric/"+ std::to_string(dof) + "dof/"; //"./dataset/scene" + path + "/";
    ...
    {   
        ...
        Planner_A_star Planner_A_star(line + line2 + std::to_string(i) + ".xml", "./sf/" + std::to_string(dof) + "dof/scene/"+line2 + std::to_string(i) + "_trajectory.xml");
        std::map<std::string, double> dict;
        bool b = Planner_A_star.AStar(start, goal, polygons, dict);
        writeDataToJson(i, dict["g_units"], dict["coord_tolerance"], dict["angle_tolerance"], dict["time"], dict["coll_check_percentage"], dict["opened_nodes"],
                        dict["closed_nodes"], dict["g_cost"], dict["turn_numbers"], "./sf/" + std::to_string(dof) + "dof/scene1_data.json");
